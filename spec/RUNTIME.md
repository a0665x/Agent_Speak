...
Legacy scripts under `scripts/` remain focused smoke and developer utilities rather than the main lifecycle interface. `./scripts/health_smoke.sh` and `./scripts/smoke_api.sh` are Docker-aware: from any working directory they first detect this repository's running Compose `gateway` and execute their Python checks inside it, so a Release install does not need a host `.venv`. When no Compose Gateway is running, they retain the local `.venv` fallback and honor the local `.env`/`API_BASE` settings. Success is reported as `HEALTH_SMOKE_OK mode=docker|local` or `API_SMOKE_OK mode=docker|local`.
