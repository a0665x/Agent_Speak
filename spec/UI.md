...
The visual system uses graphite surfaces, restrained ice-blue/violet gradients, generated speech-core artwork, system typography, and immediate press feedback. Its hero message presents the product as a local voice layer that lets any Agent listen freely and naturally join a conversation. A full-viewport Canvas 2D particle field adds spatial depth: nearby pointer movement gently repels particles and spring motion returns them to their deterministic layout. The canvas is decorative, ignores pointer events, and becomes a static composition when reduced motion is requested. Controls and links have visible keyboard focus, minimum 44 px targets, reduced-motion/transparency and increased-contrast fallbacks, and responsive layouts without horizontal page scrolling.
