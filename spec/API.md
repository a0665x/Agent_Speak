...
Resources: `GET /resources` returns the requested/resolved policy, active profile, ASR/TTS lifecycle states, and latest operation. `POST /resources/reconcile` accepts only `asr_only` or `tts_only`; `POST /resources/{workload}/reset` accepts only `asr` or `tts`. Both mutation endpoints return HTTP 202 with a bounded operation identifier. Poll `GET /resource-operations/{operation_id}` through `queued → releasing → starting → warming → ready|failed`. Resource reset follows the resolved host policy: on an exclusive host it may temporarily release the competing workload and then restore the prior ready profile; on concurrent or multi-GPU hosts it restarts only the target workload. The Gateway never receives Docker access, and these calls never activate browser audio.
