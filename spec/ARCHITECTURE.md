...
## Providers
Energy VAD is deterministic and functional. The default speech path uses local Faster-Whisper `small`; its device/compute mode is configured at runtime and its request-time language hint follows the frozen realtime session policy. Piper `zh_CN-huayan-medium` produces spoken WAV output. Both speech models load lazily. `scripts/setup.sh` installs provider packages and downloads ignored model artifacts; Faster-Whisper uses the Hugging Face cache.
